...
module.exports = {
	data: new SlashCommandBuilder()
        .setName("quiz")
        .setDescription("Answer quiz questions"),
...
class Game {
    constructor(interaction, args) { // Defining vars and running the game logic
        this.interaction = interaction
        this.args = args
        this.player = interaction.author.username
        this.reactions = ['🇦', '🇧', '🇨', '🇩']
        this.question
        this.init()
    } 
    ...
    async get_data(dif, cat) {
        ...
        if (dif && !cat) {
            ...
            if (dif.toLowerCase() != 'easy' && dif.toLowerCase() != 'medium' && dif.toLowerCase() != 'hard') return this.interaction.channel.send('Please enter a valid Difficulty\nUse .trivia categories to view a list of categories and difficulties');
        ...
        if (dif && cat) {
            ...
            if (dif.toLowerCase() != 'easy' && dif.toLowerCase() != 'medium' && dif.toLowerCase() != 'hard' && dif.toLowerCase() != 'any') return this.interaction.channel.send('Please enter a valid Difficulty\nUse .trivia categories to view a list of categories and difficulties');
            if (!this.question_id) return this.interaction.channel.send('Please enter a valid Category\nUse .trivia categories to view a list of categories and difficulties');
    ...
    async show_question() {
        ...
        this.question_interaction = await this.interaction.channel.send(this.question_embed)
        let step = -1
        while (step < this.question_length) {
            step++
            await this.question_interaction.react(this.reactions[step])
        }
        return this.await_reactions()
    }
    async await_reactions() {
        this.question_interaction.awaitReactions((reaction, user) => user.id == interaction.author.id && (reaction.emoji.name == '🇦' || reaction.emoji.name == '🇧' || reaction.emoji.name == '🇨' || reaction.emoji.name == '🇩'),
        { max: 1, time: 30000 }).then(collected => {
            ...
            if (this.input_answer == this.correct_answer) {
                ...
                .setDescription(this.answer_array)
                .setFooter('Category - ' + atob(this.question.results[0].category) + ', Difficulty - ' + atob(this.question.results[0].difficulty))
                this.question_interaction.edit(this.question_embed)
                this.question_interaction.edit('You got it correct! :smile:')
                this.end_game()
            }
            else {
                this.answer_array[this.input_answer - 1] = this.answer_array[this.input_answer - 1] + ' ❌'
                ...
                .setFooter('Category - ' + atob(this.question.results[0].category) + ', Difficulty - ' + atob(this.question.results[0].difficulty))
                this.question_interaction.edit(this.question_embed)
                this.question_interaction.edit('You got it wrong. The correct answer was ' + this.reactions[this.correct_answer - 1])
                this.end_game()
            }
        }).catch(() => {
            this.question_interaction.edit('You took to long to answer! Game has timed out. The answer was ' +  this.reactions[this.correct_answer - 1])
            this.end_game()
        })
    }
    async end_game() {
        this.question_interaction.reactions.removeAll()
        game = null
    }
}
